...
- [X] Make the control panel (Start, Stop, etc) bolder and place it above the config options.
- [X] Treat the current options area as an advanced section.
#### Minor patches
- [X] The aether fetch script fails fully if it hits a 403 rate limit; Make it wait 6 seconds and retry for 2 times
- [X] Upgrade the used actions versions to avoid Node.js deprecated warnings
- [X] Log all activities in the log box too instead of just using status bar
